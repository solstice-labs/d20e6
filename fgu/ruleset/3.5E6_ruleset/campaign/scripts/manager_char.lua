...
function addClass(nodeChar, sClass, sRecord)
	...
	local nTotalLevel = getTotalLevel(nodeChar);  
  Debug.console(nTotalLevel);
  Debug.console(sClass);
  Debug.console(sRecord);
  
  if nTotalLevel == 6 then
    local sFormatMax = Interface.getString("char_message_classmax");
    local sMsgMax = string.format(sFormatMax, "6", DB.getValue(nodeChar, "name", ""));
    ChatManager.SystemMessage(sMsgMax);
    return;
  end
...
-- @return sum of all character class levels
